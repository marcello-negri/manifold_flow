...
from enflows.transforms.injective import ConstrainedAnglesSigmoid, ClampedAngles, LearnableManifoldFlow, SphereFlow, PeriodicElementwiseTransform, ScaleLastDim
...
def generate_uniform_spherical(n, d, device="cuda"):
    assert d > 1
    samples = torch.torch.randn(n, d)
    samples /= samples.norm(dim=1).unsqueeze(-1)

    samples_sph = cartesian_to_spherical_torch(samples)
    angles = samples_sph[:, 1:].to(device)
...
def build_flow_manifold(flow_dim, n_layers=3, hidden_features=256, device='cuda'):
    ...
    transformation_layers.append(
        InverseTransform(
            LearnableManifoldFlow(n=flow_dim - 1, max_radius=5)
...
def train_model(model, data, epochs=2_001, lr=1e-3, r=1., context=None, alternating=False, dataset="spherical_uniform", epsilon=0.1, device="cuda", **kwargs):
    ...
    try:
        ...
        for epoch in range(epochs):
            ...
            if alternating:
                mse_loss = 100 * torch.norm(data_manifold - data, dim=1).mean()
                ...
                log_likelihood = -torch.mean(log_prob)
                total_loss = log_likelihood + 10 * mse_loss
                total_loss.backward()
                optimizer.step()
