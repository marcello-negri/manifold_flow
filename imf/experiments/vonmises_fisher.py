# this code is taken from the following repo: https://github.com/minyoungkim21/vmf-lib/tree/main
# which implements this paper: https://arxiv.org/abs/2102.05340
...
class vMF(nn.Module):
    ...
    def set_params(self, mu, kappa):
        ...
        with torch.no_grad():
            ...
            self.logkappa.copy_(torch.log(kappa + REAL_MIN))
...
class MixvMF(nn.Module):
    ...
    def set_params(self, alpha, mus, kappas):
        ...
        with torch.no_grad():
            self.alpha_logit.copy_(torch.log(alpha + REAL_MIN))
            for m in range(self.order):
                self.comps[m].mu_unnorm.copy_(mus[m])
                self.comps[m].logkappa.copy_(torch.log(kappas[m] + REAL_MIN))
